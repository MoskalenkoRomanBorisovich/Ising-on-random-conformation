...
from utils import read_conformation, tabulate_neighbors
...
def measure_structure(struct_1, b_min=0, b_max=1, N=10, n_sweeps=100000, num_therm=100000):
    ...
    mag_abs = np.empty(N, dtype=RealObservable)
    ...
    for i in range(N):
        ...
        ene[i], mag_abs[i], mag2[i], mag4[i], ene_arr[i] = cy_ising_cluster.simulate(neighbors=neighbors,
                                                                                     beta=betas[i],
                                                                                     num_sweeps=n_sweeps,
                                                                                     num_therm=num_therm,
    ...
    return ene, mag_abs, mag2, mag4, np.array(ene_arr)
...
def main(argv):
    ...
    ene, mag_abs, mag2, mag4, ene_arr = measure_structure(struct, b_min, b_max, N,
                                                        n_sweeps=500000,
                                                        num_therm=100000)
    betas = np.linspace(b_min+(b_max-b_min)/N, b_max, N)
    print('number of energy measurments: ', ene_arr.shape)
    np.savez(outputfile, ene=ene, mag_abs=mag_abs, mag2=mag2, mag4=mag4, betas=betas)
