...
def simulate(np.ndarray[np.int64_t, ndim=2] neighbors,
    ...
    cdef:
        ...
        int step = 0, sweep = 0
        int i, j
        double av_en = 0., Z = 0., mag_sq = 0., mag_a = 0.
        ...
        RealObservable mag_abs = RealObservable()
        list ene_arr = []
    ...
    for sweep in range(num_sweeps):
        ...
        mag_a = abs(magnetization(spins) / L)
        mag_sq = (mag_a) ** 2
        Z += 1
        ene.add_measurement(energy(spins, neighbors) / L)
        mag_abs.add_measurement(mag_a)
    ...
    return ene, mag_abs, mag2, mag4, np.array(ene_arr, dtype=RealObservable)
